...
<?php
  ...
  $infosQuarto = QuartoDao::selectAll();
  if($infosQuarto==false){
    $false = false;
  }
          ...
          <table class="table table-hover">
            <thead>
              ...
              <?php if(isset($false)){ // se o metodo de dar select nao retornou nenhuma linhas vai aparecer essa parte DAQUI
                ?> <td></td><td>Não há Quartos</td><td></td><td></td><td></td><td></td><td></td>
              <?php } //ate aqui
                else{ foreach($infosQuarto as $info){?> <!-- SE RETORNOU LINHAS IREMOS EXIBIR NA TELA -->
              <tr>
                <td><?=$info['codQuarto']?></td>
                <td><?=$info['nomeQuarto']?></td>
                <td><?=$info['descQuarto']?></td>
                <td><?=$info['nomeCategoriaQuarto']?></td>
               
                <td class="text-center">
                  <form action="./process.php" method="POST">
                    <input type="hidden" value="<?=$info['codQuarto']?>" name="cod">
                    <button type="submit" value="SELECTID" name="acao" class="dropdown-item"><i class="fas fa-edit fa-lg text-secondary"></i>
                    </button>
                  </form>
                </td>
                <td class="text-center">
                  <a class="dropdown-item" onclick="modalRemover(<?=$info['codQuarto']?>,'codDeletar')">
