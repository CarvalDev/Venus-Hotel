...
<?php
  ...
  $infoReserva = ReservaDao::selectAll();
  if($infoReserva==false){
    $false = false;
  } 
          ...
          <table class="table table-hover">
            <thead>
              <tr>
                ...
                <th class="col-md-3">Data de Agendamento </th>
                <th class="col-md-3">ID Quarto</th>
                <th class="col-md-3">ID Cliente</th>
              ...
              <?php } //ate aqui
                else{ foreach($infoReserva as $info){?> <!-- SE RETORNOU LINHAS IREMOS EXIBIR NA TELA -->
              <tr>
                <td><?=$info['codReserva']?></td>
                <td><?=$data = date('d/m/Y H:i:s', strtotime($info['dataHoraReserva']));?></td>
                <td><?=$info['codQuarto']?></td>
                <td><?=$info['codCliente']?></td>
                <td><?=$info['statusReserva']?></td>
               
                <td class="text-center">
                  <form action="./process.php" method="POST">
                    <input type="hidden" value="<?=$info['codReserva']?>" name="cod">
